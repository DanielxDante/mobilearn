...
const Review = () => {
  ...
  const handleSubmit = async () => {
    if (feedback?.length === 0) {
      alert(t("reviewPageConstants.enterFeedback"));
    } else {
      const response = await saveReview(Number(courseId), rating, feedback);
      if (response === true) {
        Alert.alert(
          t("reviewPageConstants.success"),
          t("reviewPageConstants.reviewSubmitted"),
          [
            {
              text: t("reviewPageConstants.ok"),
              onPress: () => router.back(),
            },
          ],
          { cancelable: false }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.titleContainer}>
        <Text style={styles.title}>{t("reviewPageConstants.title")}</Text>
      </View>
      {/* Body */}
      <View style={styles.body}>
        ...
        <View style={styles.feedbackContainer}>
          <TextInput
            ...
            value={feedback}
            placeholder={t("reviewPageConstants.placeholder")}
          />
        </View>
        <View>
          <TouchableOpacity
