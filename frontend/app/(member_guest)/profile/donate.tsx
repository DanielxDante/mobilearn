...
const Donate = () => {
    ...
    const fetchStripePaymentSheet = useAppStore((state) => state.fetchPaymentSheet);
    ...
    const handlePayment = async (donationAmount: number, default_currency: string) => {
        setLoading(true);
        try {
            const { payment_intent, ephemeral_key, customer_id } = await fetchStripePaymentSheet(donationAmount.toString(), default_currency);
            ...
            if (error) {
                Alert.alert("Payment cancelled", "You have cancelled the donation.");
            } else {
                Alert.alert("Payment successful", "Thank you for your donation!");
                router.back();
            }
        } catch (error) {
            Alert.alert("Error", "An error occurred while processing your donation. Please try again later.");
        } finally {
            setLoading(false);
        }
    };
                ...
                <View style={styles.body}>
                    <View style={styles.upperBody}>
                        ...
                        <Text style={styles.donateText}>
                            {Constants.donateText1}
                        </Text>
                        <Text style={styles.donateText}>
                            {Constants.donateText2}
                        </Text>
                    </View>
                    <View style={styles.lowerBody}>
                            ...
                            <TextInput
                                ...
                                onChangeText={(text) => {
                                    ...
                                    if (decimalIndex >= 0) {
                                        ...
                                        const decimalPart = formattedText.slice(decimalIndex + 1, decimalIndex + 3);
                                        setAmount(`${integerPart}.${decimalPart}`);
                                    } else {
                                        setAmount(formattedText);
                                    }
                                    setDonateButtonDisabled(!formattedText || parseFloat(formattedText) <= 0);
                                }}
                                placeholder="0"
                                maxLength={10}
                                onFocus={() => {
                        ...
                        {loading && (
                            <ActivityIndicator size="large" color={Colors.defaultBlue} style={styles.spinner} />
                        )}
                    </View>
                </View>
            </ScrollView>
